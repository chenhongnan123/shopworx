      ...
      <v-row style="padding:.12rem 0 0 .16rem;">
        ...
        <v-col cols="5">
          ...
          <v-progress-circular
            ...
            :color="'#55D802'"
          >
            <div style="width:2rem;color:#fff;font-size:.3rem;word-wrap: break-word;text-align: center;line-height: .4rem;font-weight:700;">
  ...
  watch: {
    confidenceData: {
      handler(confidenceData) {
        ...
        //   confidenceStatus = 0;
        // } else if (confidenceData.overallconfidence > confidenceData.badthresholdpercent && confidenceData.overallconfidence <= confidenceData.goodthresholdpercent) {
        //   confidenceStatus = 1;
        // } else {
        //   confidenceStatus = 2;
        // }
        ...
        const okCount = confidencebyoperation.filter(i => i.prediction === 1).map(i => i.predictioncount).sort((a, b) => a-b)[0];
        const ngCount = confidencebyoperation.filter(i => i.prediction === -1).map(i => i.predictioncount).sort((a, b) => b-a)[0];
